...
def get_function_name_from_definition_c(node):
    """
    Extracts the function name from a C function definition AST node.
    
    This function traverses an abstract syntax tree (AST) structure generated by a C parser 
    to extract the identifier corresponding to a function definition. It handles common
    C syntax patterns including declaration specifiers (e.g., static/extern), pointer
    declarators, and function parameter lists. The implementation specifically targets AST
    structures produced by the [insert relevant parser library if found] tooling.
    
    Args:
        node: Root AST node containing a C function definition structure. Expected to have
            children with types like 'declaration_specifiers', 'pointer_declarator',
            and 'function_declarator' as per the parser's grammar rules.
    
    Returns:
        str or None: Decoded Unicode string of the function name if found, otherwise None.
        Returns None for non-function definitions or malformed AST structures.
    
    Note:
        - Skips storage class specifiers (static/inline/extern) during traversal
        - Handles single-level pointer declarators (not multiple indirections)
        - Used by [list key caller functions if found] for symbol resolution
        - Assumes identifier is a UTF-8 encoded byte string in AST node
    """
...
def get_function_name_from_definition_py(node):
    """
    Extracts the function name identifier from a Python AST node representing a function definition.
    
    Args:
        node: A tree-sitter Node object representing a function definition, expected to have 
            a 'name' field pointing to an identifier child node.
    
    Returns:
        str or None: The decoded text of the identifier child node if found, otherwise None.
    
    Note:
        Relies on the node structure having a 'name' field with an identifier child. 
        Typically used for parsing Python function definitions in ASTs. 
        Functions like parse_ast() and analyze_code() may depend on this method to extract identifiers.
    
    Example:
        When analyzing AST nodes from Python source code, this method helps retrieve the actual 
        function name for further processing in code analysis tasks.
    """
    for child in node.children:
...
LANGUAGE_HANDLERS = {
    "c": {
        ...
        "name_extractor": get_function_name_from_definition_c,
    },
    "python": {
        "func_def_type": "function_definition",
        "body_type": "block",
        "call_type": "call",
        "name_extractor": get_function_name_from_definition_py,
    },
...
def fetch_source_snippet(fn_id, G, metadata=None, repo_path="", file_hint=None):
    ...
    def find_source_block(node):
        if node.type == "function_definition":
            handler = LANGUAGE_HANDLERS.get(lang)
            name = handler["name_extractor"](node)
            if name is not None and name == func_name:
                # print("Found function declaration. Get source.")
                return node.text.decode()
        for child in node.children:
            source = find_source_block(child)
    ...
    try:
        ...
        return find_source_block(tree.root_node)
